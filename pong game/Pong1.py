...
def cpu2move(cpu2,cpu3,ball):
    if ball.movement[0] > 0: #ensures that the CPU moves only when the ball is directed towards it
        if ball.rect.left > cpu2.rect.left + cpu2.rect.width/6:
            cpu2.movement[0] = 8
            cpu3.movement[0] = 8
        elif ball.rect.right < cpu2.rect.right - cpu2.rect.width/6:
            cpu2.movement[0] = -8
            cpu3.movement[0] = -8
        else:
            cpu2.movement[0] = 0
...
The class is initialized by passing 5 parameters
...
class Paddle(pygame.sprite.Sprite):
    def __init__(self,x,y,sizex,sizey,color):
        ...
        pygame.draw.rect(self.image, self.color, (sizex, sizey, 0, 0))
...
class Paddle_cpu(pygame.sprite.Sprite):
    def __init__(self,x,y,sizex,sizey,color):
        pygame.sprite.Sprite.__init__(self)
        self.x = x
        self.y = y
...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self):
        if (self.rect.top == 0 and self.rect.left <= width /2) or (self.rect.bottom == height and self.rect.left <=width /2): #reverses the vertical velocity on collision with top and bottom walls
            #self.movement[1] = -1*self.movement[1]       // remove this if success
...
def main():
    ...
    paddle = Paddle(width/10,height/2,width/60,height/8,green) #creating an object for the user's paddle
    paddle2 =Paddle(width/4,height/30,width/9,height/50,green)
    paddle3 =Paddle(width/4,height-15,width/9,height/50,green)
    cpu = Paddle_cpu(width - width/10,height/2,width/60,height/8,white) #creating an object for the CPU's paddle
    cpu2 = Paddle_cpu(width - width / 2, height -390, width / 9, height / 50, white)
    cpu3 = Paddle_cpu(width - width / 2, height-15 , width / 9, height / 50, white)
    ...
    while not gameOver: #running our game loop
        for event in pygame.event.get(): #checks for various events in pygame, like keypress, mouse movement, etc
            ...
            if event.type == pygame.KEYDOWN: #checks whether a key has been pressed or not
                if event.key == pygame.K_UP: #If user has pressed the UP key
                    ...
                    paddle2.movement[0] = 8
                    paddle3.movement[0] = 8
                elif event.key == pygame.K_LEFT:  # If user has pressed the down key
                    paddle2.movement[0] = -8
                    paddle3.movement[0] = -8

            if event.type == pygame.KEYUP:    #If the user lifts the key
                paddle.movement[1] = 0        #Paddle stops moving
                paddle2.movement[0] = 0
                paddle3.movement[0] = 0

        cpumove(cpu,ball) #moves the CPU's paddle

        cpu2move(cpu2,cpu3, ball)
        ...
        if pygame.sprite.collide_mask(paddle, ball):
          ...
          # for  top horizital paddles
        if pygame.sprite.collide_mask(paddle2, ball):
            pygame.mixer.Sound.play(collide_sound)
            ball.movement[0] = -1*ball.movement[0]
            ball.movement[1] = -1*ball.movement[1] + int(0.3*random.randrange(5,10))
            if ball.movement[1] > ball.maxspeed:
                ball.movement[1] = ball.maxspeed
            if ball.movement[1] < -1*ball.maxspeed:
                ball.movement[1] = -1*ball.maxspeed

                # for  bottom horizital paddles
        if pygame.sprite.collide_mask(paddle3, ball):
            pygame.mixer.Sound.play(collide_sound)
            ball.movement[0] = -1 * ball.movement[0]
            ball.movement[1] =-1* ball.movement[1] + int(0.3*random.randrange(5,10))
            if ball.movement[1] > ball.maxspeed:
        ...
        if pygame.sprite.collide_mask(cpu2,ball):
            pygame.mixer.Sound.play(collide_sound)
            ball.movement[0] = -1*ball.movement[0]
            ball.movement[1] = -1*ball.movement[1] + int(0.3*random.randrange(5,10))
            if ball.movement[1] > ball.maxspeed:
        ...
        if pygame.sprite.collide_mask(cpu3, ball):
            pygame.mixer.Sound.play(collide_sound)
            ball.movement[0] = -1*ball.movement[0]
            ball.movement[1] = -1*ball.movement[1] + int(0.3*random.randrange(5,10))
            if ball.movement[1] > ball.maxspeed:
